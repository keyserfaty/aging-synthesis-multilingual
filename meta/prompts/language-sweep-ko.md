# Korean Language Sweep

You are running a weekly sweep of Korean aging-intervention literature for the Round 2 multilingual synthesis at `~/aging-synthesis-multilingual/`. This task is autonomous; no human is in the loop.
...
3. Search Korean aging literature published in the last 7 days (or since the last successful sweep — check `~/aging-synthesis-multilingual/meta/sweep-log-ko.md` for last-completed timestamp). Use available tools to query KISS, RISS, KMbase where reachable. If no MCP/API access is available for these databases, fall back to:
   - Google Scholar with Korean-language filter and aging-relevant search terms (노화, 장수, 항노화, 노년의학, 수명)
   - PubMed for Korean-language abstracts (filter `lang:kor`)
...
7. Append to `~/aging-synthesis-multilingual/meta/sweep-log-ko.md`: timestamp, items found, items added to queue, items rejected (with reason).
8. Commit: `cd ~/aging-synthesis-multilingual && git add meta/queue.md meta/translation-provenance.md meta/sweep-log-ko.md && git commit -m "ko sweep <date>: N items added" && git push`.

## Queue item schema

```yaml
- id: ko-<sweep-date>-<seq>
  language: ko
  intervention_normalized: <english intervention name, snake_case>
  intervention_native: "<Korean name>"
...
If a database is unreachable, log to `~/.claude/logs/aging-ml-language-sweep-ko.err.log` and proceed with whatever sources are reachable. Do not crash the sweep.

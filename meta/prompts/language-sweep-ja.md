# Japanese Language Sweep

You are running a weekly sweep of Japanese aging-intervention literature for the Round 2 multilingual synthesis at `~/aging-synthesis-multilingual/`. This task is autonomous; no human is in the loop.
...
3. Search Japanese aging literature published in the last 7 days (or since the last successful sweep — check `~/aging-synthesis-multilingual/meta/sweep-log-ja.md` for last-completed timestamp). Use available tools to query J-STAGE, CiNii, ICHUSHI where reachable. If no MCP/API access is available for these databases, fall back to:
   - Google Scholar with Japanese-language filter and aging-relevant search terms (老化, 加齢, 寿命, 長寿, 健康寿命, 抗加齢)
   - PubMed for Japanese-language abstracts (filter `lang:jpn`)
...
7. Append to `~/aging-synthesis-multilingual/meta/sweep-log-ja.md`: timestamp, items found, items added to queue, items rejected (with reason).
8. Commit: `cd ~/aging-synthesis-multilingual && git add meta/queue.md meta/translation-provenance.md meta/sweep-log-ja.md && git commit -m "ja sweep <date>: N items added" && git push`.

## Queue item schema

```yaml
- id: ja-<sweep-date>-<seq>
  language: ja
  intervention_normalized: <english intervention name, snake_case>
  intervention_native: "<Japanese name>"
...
If a database is unreachable, log to `~/.claude/logs/aging-ml-language-sweep-ja.err.log` and proceed with whatever sources are reachable. Do not crash the sweep.

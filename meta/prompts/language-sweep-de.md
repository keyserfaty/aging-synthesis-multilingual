# German Language Sweep

You are running a weekly sweep of German aging-intervention literature for the Round 2 multilingual synthesis at `~/aging-synthesis-multilingual/`. This task is autonomous; no human is in the loop.
...
3. Search German aging literature published in the last 7 days (or since the last successful sweep — check `~/aging-synthesis-multilingual/meta/sweep-log-de.md` for last-completed timestamp). Use available tools to query LIVIVO, GMS, DIMDI where reachable. If no MCP/API access is available for these databases, fall back to:
   - Google Scholar with German-language filter and aging-relevant search terms (Alterung, Langlebigkeit, Anti-Aging, Geriatrie, Gerontologie, Lebensspanne)
   - PubMed for German-language abstracts (filter `lang:ger`)
...
7. Append to `~/aging-synthesis-multilingual/meta/sweep-log-de.md`: timestamp, items found, items added to queue, items rejected (with reason).
8. Commit: `cd ~/aging-synthesis-multilingual && git add meta/queue.md meta/translation-provenance.md meta/sweep-log-de.md && git commit -m "de sweep <date>: N items added" && git push`.

## Queue item schema

```yaml
- id: de-<sweep-date>-<seq>
  language: de
  intervention_normalized: <english intervention name, snake_case>
  intervention_native: "<German name>"
...
If a database is unreachable, log to `~/.claude/logs/aging-ml-language-sweep-de.err.log` and proceed with whatever sources are reachable. Do not crash the sweep.

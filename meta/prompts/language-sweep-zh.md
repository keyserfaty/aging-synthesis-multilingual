# Chinese (Mandarin) Language Sweep

You are running a weekly sweep of Mandarin-Chinese aging-intervention literature for the Round 2 multilingual synthesis at `~/aging-synthesis-multilingual/`. This task is autonomous; no human is in the loop.
...
3. Search Mandarin-Chinese aging literature published in the last 7 days (or since the last successful sweep — check `~/aging-synthesis-multilingual/meta/sweep-log-zh.md` for last-completed timestamp). Use available tools to query CNKI, Wanfang Data, CMB, SinoMed where reachable. If no MCP/API access is available for these databases, fall back to:
   - Google Scholar with Chinese-language filter and aging-relevant search terms (老化, 衰老, 长寿, 健康衰老, 抗衰老, 寿命, 老年医学)
   - PubMed for Chinese-language abstracts (filter `lang:chi`)
...
7. Append to `~/aging-synthesis-multilingual/meta/sweep-log-zh.md`: timestamp, items found, items added to queue, items rejected (with reason).
8. Commit: `cd ~/aging-synthesis-multilingual && git add meta/queue.md meta/translation-provenance.md meta/sweep-log-zh.md && git commit -m "zh sweep <date>: N items added" && git push`.

## Queue item schema

```yaml
- id: zh-<sweep-date>-<seq>
  language: zh
  intervention_normalized: <english intervention name, snake_case>
  intervention_native: "<Chinese name>"
...
If a database is unreachable, log to `~/.claude/logs/aging-ml-language-sweep-zh.err.log` and proceed with whatever sources are reachable. Do not crash the sweep.

# Spanish + French Language Sweep

You are running a weekly sweep of Spanish and French aging-intervention literature for the Round 2 multilingual synthesis at `~/aging-synthesis-multilingual/`. This task is autonomous; no human is in the loop.
...
3. Search Spanish and French aging literature published in the last 7 days (or since the last successful sweep — check `~/aging-synthesis-multilingual/meta/sweep-log-es-fr.md` for last-completed timestamp). Use available tools to query LILACS, SciELO, Dialnet, Pascal-Francis, HAL, Cairn where reachable. If no MCP/API access is available for these databases, fall back to:
   - Google Scholar with Spanish or French-language filter and aging-relevant search terms (envejecimiento, longevidad, antienvejecimiento, geriatría, vieillissement, longévité, antivieillissement, gérontologie)
   - PubMed for Spanish or French-language abstracts (filter `lang:spa OR lang:fre`)
...
7. Append to `~/aging-synthesis-multilingual/meta/sweep-log-es-fr.md`: timestamp, items found, items added to queue, items rejected (with reason).
8. Commit: `cd ~/aging-synthesis-multilingual && git add meta/queue.md meta/translation-provenance.md meta/sweep-log-es-fr.md && git commit -m "es+fr sweep <date>: N items added" && git push`.

## Queue item schema

```yaml
- id: es-fr-<sweep-date>-<seq>
  language: <es|fr>
  intervention_normalized: <english intervention name, snake_case>
  intervention_native: "<Spanish or French name>"
...
If a database is unreachable, log to `~/.claude/logs/aging-ml-language-sweep-es-fr.err.log` and proceed with whatever sources are reachable. Do not crash the sweep.
